...
public:
   ...
   RMM( const map<string, double> &, const vector<string> &, const LM &, const LM &, const LM & );
   void train( int );
   void train( const vector<int> & );
...
RMM::RMM( const map<string, double> &ALPHA, const vector<string> &DOCLIST, const LM &Rmodel, const LM &Bmodel, const LM &Qmodel )
{
   alpha = ALPHA;
   doclist = DOCLIST;
   R = ltom( Rmodel );
...
void RMM::train( int iter )
{
   for( int i = 0; i < iter; i++ )
      EM();
}

void RMM::train( const vector<int> &mulist )
{
   int iter = mulist.size();
